...
<div class="card">
	<div class="card-header d-flex justify-content-between flex-column flex-sm-row gap-2">
		<div>
			...
			<a href="{{ route('export', ['module' => 'commissions', 'format' => 'pdf']) }}" class="btn btn-outline-danger" target="_blank" data-bs-toggle="tooltip" title="Exportar a PDF">
				<i class="ti ti-file-type-pdf icon"></i> PDF
			</a>
			<a href="{{ route('export', ['module' => 'commissions', 'format' => 'excel']) }}" class="btn btn-outline-success" data-bs-toggle="tooltip" title="Exportar a Excel">
	...
	<div class="table-responsive">
		<table class="table card-table table-vcenter">
			...
			<tbody>
				...
				<tr>
					<td>{{ $commission->dni }}</td>
					<td>{{ $commission->names }}</td>
					<td>{{ $commission->phone }}</td>
					<td>{{ $commission->observation }}</td>
					<td>
						<div class="d-flex gap-2">
							<div class="d-flex gap-2">
								<button class="btn btn-icon btn-primary btn-edit " data-id="{{ $commission->id }}" data-bs-toggle="tooltip" title="Editar">
									<i class="ti ti-pencil icon"></i>
								</button>
								<button class="btn btn-icon btn-red btn-delete" data-id="{{ $commission->id }}" data-bs-toggle="tooltip" title="Eliminar">
  			  ...
  			  <div class="row">
  			  	...
  			  	<div class="col-lg-6">
  			  		<div class="mb-3">
  			  			...
  			  			<input type="text" class="form-control" name="names" autocomplete="off" required>
  			  		</div>
  			  	</div>
  			  	<div class="col-lg-6">
  			  		<div class="mb-3">
  			...
  			<div class="modal-body">
  			  <div class="row">
  			  	<div class="col-lg-6">
  			  		<div class="mb-3">
  			  			<label class="form-label">DNI</label>
  			  			<input type="text" class="form-control" name="dni" id="editDni" autocomplete="off" required>
  			  		</div>
  			  	</div>
  			  	<div class="col-lg-6">
  			  		<div class="mb-3">
  			  			<label class="form-label">Nombres</label>
  			  			<input type="text" class="form-control" name="names" id="editNames" autocomplete="off" required>
  			  		</div>
  			  	</div>
  			  	<div class="col-lg-6">
  			  		<div class="mb-3">
  			  	...
  			  	<div class="col-lg-12">
  			  		<div class="mb-3">
  			  			<label class="form-label">Observación</label>
  			  			<textarea class="form-control" name="observation" id="editObservation" rows="3"></textarea>
  			  		</div>
  			  	</div>
  			  </div>
  			</div>
...
<script>
	...
	$(document).on('click', '.btn-edit', function(){
		...
		$.ajax({
			url: '{{ route('commissions.index') }}' + '/' + id + '/edit/',
			method: 'GET',
			success: function(data){
	...
	$('#editForm').submit(function(e){
		...
		$.ajax({
			url: '{{ route('commissions.index') }}' + '/' + id + '',
			method: 'PATCH',
			data: $(this).serialize(),
			success: function(data){
				if(data.status){
