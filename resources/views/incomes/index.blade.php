...
<div class="card">
	<div class="card-header d-flex justify-content-between flex-column flex-sm-row gap-2">
		<div>
			...
			<a href="{{ route('export', ['module' => 'incomes', 'format' => 'pdf']) }}" class="btn btn-outline-danger" target="_blank" data-bs-toggle="tooltip" title="Exportar a PDF">
				<i class="ti ti-file-type-pdf icon"></i> PDF
			</a>
			<a href="{{ route('export', ['module' => 'incomes', 'format' => 'excel']) }}" class="btn btn-outline-success" data-bs-toggle="tooltip" title="Exportar a Excel">
		...
		<table class="table card-table table-vcenter">
			...
			<tbody>
				...
				<tr>
					...
					<td>{{ $income->amount }}</td>
					<td>{{ optional($income->income_type)->name }}</td>
					<td>{{ optional($income->payment_method)->name }}</td>
					<td>{{ optional($income->location)->name }}</td>
					<td>{{ $income->date->format('d/m/Y') }}</td>
					<td>
						<div class="d-flex gap-2">
							<div class="d-flex gap-2">
								<button class="btn btn-icon btn-primary btn-edit" data-id="{{ $income->id }}" data-bs-toggle="tooltip" title="Editar">
									<i class="ti ti-pencil icon"></i>
								</button>
								<button class="btn btn-icon btn-red btn-delete" data-id="{{ $income->id }}" data-bs-toggle="tooltip" title="Eliminar">
					...
					<div class="row">
						...
						<div class="col-lg-6">
							<div class="mb-3">
								...
								<select class="form-select" name="income_type_id">
				...
				<div class="modal-body">
					<div class="row">
						...
						<div class="col-lg-6">
							<div class="mb-3">
								<label class="form-label required">Tipo de ingreso</label>
								<select class="form-select" name="income_type_id" id="editIncomeTypeId">
									<option value="">Seleccionar</option>
									@foreach($income_types as $income_type)
									<option value="{{ $income_type->id }}">{{ $income_type->name }}</option>
									@endforeach
	...
	$(document).on('click', '.btn-edit', function(){
		...
		$.ajax({
			...
			success: function(data){
				...
				$('#editIncomeTypeId').val(data.income_type_id);
