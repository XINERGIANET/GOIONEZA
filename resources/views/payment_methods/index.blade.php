...
<div class="card">
	<div class="card-header d-flex justify-content-between flex-column flex-sm-row gap-2">
		<div>
			...
			<a href="{{ route('export', ['module' => 'payment_methods', 'format' => 'pdf']) }}" class="btn btn-outline-danger" target="_blank" data-bs-toggle="tooltip" title="Exportar a PDF">
				<i class="ti ti-file-type-pdf icon"></i> PDF
			</a>
			<a href="{{ route('export', ['module' => 'payment_methods', 'format' => 'excel']) }}" class="btn btn-outline-success" data-bs-toggle="tooltip" title="Exportar a Excel">
				...
				<tr>
					...
					<td>
						<div class="d-flex gap-2">
							<div class="d-flex gap-2">
								<button class="btn btn-icon btn-primary btn-edit" data-id="{{ $payment_method->id }}" data-bs-toggle="tooltip" title="Editar">
									<i class="ti ti-pencil icon"></i>
								</button>
								<button class="btn btn-icon btn-red btn-delete" data-id="{{ $payment_method->id }}" data-bs-toggle="tooltip" title="Desactivar">
	...
	<div class="card-footer d-flex align-items-center">
		{{ $payment_methods->withQueryString()->links() }}
	</div>
	@endif
</div>
  	...
  	<div class="modal-content">
  		<form id="storeForm" method="POST">
  			<div class="modal-header">
  			  <h5 class="modal-title">Crear nuevo método de pago</h5>
  			...
  			<div class="modal-body">
  			  <div class="row">
  			  	<div class="col-lg-12">
  			  		<div class="mb-3">
  			  			<label class="form-label required">Nombre</label>
  			  			<input type="text" class="form-control" name="name" id="editName" autocomplete="off" required>
  			  		</div>
  			  	</div>
  			  </div>
  			</div>
...
<script>
	...
	$(document).on('click', '.btn-edit', function(){
		...
		$.ajax({
			url: '{{ route('payment_methods.index') }}' + '/' + id + '/edit/',
			method: 'GET',
			success: function(data){
	...
	$('#editForm').submit(function(e){
		...
		$.ajax({
			url: '{{ route('payment_methods.index') }}' + '/' + id + '',
			method: 'PATCH',
			data: $(this).serialize(),
			success: function(data){
				if(data.status){
	...
	$(document).on('click', '.btn-delete', function(){
		...
		ToastConfirm.fire({
			text: '¿Estás seguro que deseas desactivar este método de pago?',
		}).then((result) => {
			...
			if(result.isConfirmed){
				...
				$.ajax({
					...
					method: 'DELETE',
					success: function(data){
						ToastMessage.fire({ text: 'Registro desactivado' })
							.then(() => location.reload());
					},
					error: function(err){
						ToastError.fire({ text: 'Ocurrió un error' });
