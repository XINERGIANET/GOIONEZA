...
<div class="card">
	<div class="card-header d-flex justify-content-between flex-column flex-sm-row gap-2">
		<div>
			...
			<a href="{{ route('export', ['module' => 'event_types', 'format' => 'pdf']) }}" class="btn btn-outline-danger" target="_blank" data-bs-toggle="tooltip" title="Exportar a PDF">
				<i class="ti ti-file-type-pdf icon"></i> PDF
			</a>
			<a href="{{ route('export', ['module' => 'event_types', 'format' => 'excel']) }}" class="btn btn-outline-success" data-bs-toggle="tooltip" title="Exportar a Excel">
				...
				<tr>
					...
					<td>
						<div class="d-flex gap-2">
							<div class="d-flex gap-2">
								<button class="btn btn-icon btn-primary btn-edit " data-id="{{ $event_type->id }}" data-bs-toggle="tooltip" title="Editar">
									<i class="ti ti-pencil icon"></i>
								</button>
								<button class="btn btn-icon btn-red btn-delete" data-id="{{ $event_type->id }}" data-bs-toggle="tooltip" title="Eliminar">
	...
	@if($event_types->hasPages())
	<div class="card-footer d-flex align-items-center">
		{{ $event_types->withQueryString()->links() }}
	</div>
	@endif
</div>
...
<script>
	...
	$(document).on('click', '.btn-edit', function(){
		...
		$.ajax({
			url: '{{ route('event_types.index') }}' + '/' + id + '/edit/',
			method: 'GET',
			success: function(data){
	...
	$('#editForm').submit(function(e){
		...
		$.ajax({
			url: '{{ route('event_types.index') }}' + '/' + id + '',
			method: 'PATCH',
			data: $(this).serialize(),
			success: function(data){
				if(data.status){
